...
- [x] 4.2. Определить доменные сущности `Order`, `OrderItem`, `CustomerInfo`, `OrderStatus` (Pending, Confirmed, Shipped, Delivered, Cancelled).
...
- [x] 4.4. Реализовать бизнес-логику расчета стоимости:
  ...
  - Поддержка промокодов (`WARHAMMER10`, `TERRA10`, `WARP-TITHE-10` — скидка 10%, `EMPEROR20` — скидка 20%).
  - Подготовка к внедрению модуля накопительных скидок лояльности (регламентная работа ПМ.03).
