<script lang="ts">
  ...
  const badgeStyle = $derived(
    normalized.includes('chaos')
      ...
      : normalized.includes('necron')
      ? 'border-emerald-800 text-emerald-300 bg-emerald-950/30'
      : normalized.includes('xenos')
      ? 'border-teal-800 text-teal-300 bg-teal-950/30'
...
<span class="vault-badge {badgeStyle} {className}">
  ...
  {:else if normalized.includes('order') || normalized.includes('sigmar')}
    <svg class="w-3.5 h-3.5 inline-block text-sky-300 shrink-0" viewBox="0 0 24 24" fill="currentColor">
      <path d="M12 2l3 7h7l-5.5 4.5 2 7L12 17l-6.5 3.5 2-7L2 9h7l3-7z"></path>
    </svg>
  {:else if normalized.includes('xenos')}
    <svg class="w-3.5 h-3.5 inline-block text-teal-400 shrink-0" viewBox="0 0 24 24" fill="currentColor">
      <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 13h4v2h-4v-2zm-2-4a2 2 0 114 0 2 2 0 01-4 0zm8 0a2 2 0 114 0 2 2 0 01-4 0z"></path>
    </svg>
  {:else}
